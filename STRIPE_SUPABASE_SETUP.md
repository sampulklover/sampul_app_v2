...
- **Set Product metadata (recommended)**  
  - On each product's page, scroll down to **Metadata** section → click **Add metadata**.  
  - Add key: `ref_product_key` with value:
    - `P0001` for free plans (price = 0)
    - `P0003` for paid plans (price > 0)
  - This metadata will be automatically passed to subscriptions, making it easy to manage product keys without updating Edge Function code.  
  - **Note:** Using Product metadata (instead of Price metadata) is better if you have multiple prices per product (e.g., monthly/yearly), as they'll all share the same `ref_product_key`.  
- **Get Price IDs**  
...
### 7. Configure deep links for return URLs (iOS & Android)
- **iOS (`ios/Runner/Info.plist`):**
  - Add the URL scheme to `CFBundleURLTypes`:
    ```xml
...
- **Android (`android/app/src/main/AndroidManifest.xml`):**
  - Add an intent filter to the main activity:
    ...
    <activity android:name=".MainActivity" ...>
      ...
      <!-- Deep link intent filter for Stripe return URLs -->
      <intent-filter>
        <action android:name="android.intent.action.VIEW"/>
        <category android:name="android.intent.category.DEFAULT"/>
        <category android:name="android.intent.category.BROWSABLE"/>
        <data android:scheme="sampul"/>  <!-- Must match STRIPE_RETURN_URL_SCHEME in .env -->
      </intent-filter>
    </activity>
    ```
- **Note:** The `sampul` scheme in both platforms must match the value of `STRIPE_RETURN_URL_SCHEME` in your `.env` file. After payment, Stripe will redirect to `sampul://billing/success`, which opens your app automatically.
...
Deno.serve(async (req) => {
  try {
    ...
    // Retrieve price and product to get metadata (e.g., ref_product_key from product)
    const price = await stripe.prices.retrieve(priceId);
    const productId = typeof price.product === "string" ? price.product : price.product?.id;
    let refProductKey: string | undefined;
    if (productId) {
      const product = await stripe.products.retrieve(productId);
      refProductKey = product.metadata?.ref_product_key;
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      subscription_data: {
        metadata: {
          ...
          ...(refProductKey && { ref_product_key: refProductKey }),
        },
      },
      metadata: { supabase_user_id: user.id },
    });
...
Deno.serve(async (req) => {
  try {
    ...
    if (event.type.startsWith("customer.subscription.")) {
      ...
      const refProductKey = sub.metadata?.ref_product_key as string | undefined;
      ...
      if (supabaseUserId) {
        const isSubscribed = !["canceled", "incomplete_expired"].includes(
          sub.status,
        );
        ...
        await supabase.from("accounts").upsert(
          {
            ...
            stripe_customer:
              typeof sub.customer === "string" ? sub.customer : undefined,
            // Clear subscription fields if canceled, otherwise keep them
            stripe_price_id: isSubscribed ? price?.id : null,
            stripe_product: isSubscribed
              ? (typeof price?.product === "string" ? price.product : undefined)
              : null,
            stripe_interval: isSubscribed
              ? price?.recurring?.interval ?? null
              : null,
            is_subscribed: isSubscribed,
            ref_product_key: isSubscribed ? refProductKey ?? undefined : null,
          },
          { onConflict: "uuid" },
        );
      }
